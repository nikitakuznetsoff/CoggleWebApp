...
def upload_file():
    ...
    <form method=post enctype=multipart/form-data>
        <input type=radio id="contactChoice1" name="form_algo" value="without_ref" checked>
                <label for="contactChoice1">Сравнение двух интеллект карт</label>
...
def take_mm(filename, status = None):
    ...
    if status is None:
        ...
        for i in range(0, len(arr_ids) - 1):
            for j in range(0, len(arr_ids) - 1):
        ...
        gf.print_matrix(output_sheet, mass, len(arr_ids))
        wb.save(filename)
